...
export class BAUFetchProvider implements IFetchProvider {
  ...
  async fetchDepartments(collegeId: string): Promise<DepartmentApiResponse[]> {
    return this.request<DepartmentApiResponse[]>("getDepartments", [
      Number(collegeId),
    ]);
  }
  ...
  async fetchCourses(
    ...
    return this.request<CourseApiResponse[]>("getCourses", [
      Number(degreeId),
      Number(collegeId),
      Number(departmentId),
    ]);
  ...
  async fetchSections(
    ...
    return this.request<SectionApiResponse[]>("getCourses", [
      Number(degreeId),
      Number(collegeId),
      Number(departmentId),
    ]);
